...
function showurls() {
  ...
  if (scroll < -height / 4 + (servers.names.length * height) / 6) {
    scroll = -height / 4 + (servers.names.length * height) / 6;
  }
  translate(0, scroll);
...
function create() {
  ...
  fill(255, 255, 255, 20);
  rect(0, height / 10, width, height - height / 5);
  ...
  strokeWeight(5);
  stroke(255, 0, 0);
  rect(width / 4, height * 0.6, width - width / 2, height / 15, 10);
  noStroke();

  textSize(s * 17);
  fill(255, 0, 0);
  text("SUBMIT", width / 4, height * 0.6, width - width / 2, height / 15);
  if (incorrect) {
  ...
  if (
    key === "Enter" ||
    (button(width / 4, height * 0.6, width - width / 2, height / 15) &&
      mouseIsPressed)
  ) {
    if (
      (setting === "ENTER URL" &&
...
function draw() {
  ...
  if (stage === "ADMIN") {
    text("ADMIN CONTROLS", width / 2, height / 15);
    if (
      mouseY<height/10 &&
      mouseIsPressed &&
      open
